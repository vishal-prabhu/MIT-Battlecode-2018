...
def knight_actions(unit_id):
    if not gc.can_sense_unit(unit_id):
        return
    this_unit = gc.unit(unit_id)

    config.my_units = gc.my_units()
    knight_actions.attack_moving = False
    knight_actions.boarded_rocket = False
    knight_actions.moving_to_board_rocket = False
    ...
    if gc.round() > config.rocket_launch_round and gc.planet() == bc.Planet.Earth:
        knight_actions.boarded_rocket, knight_actions.moving_to_board_rocket = Utilities.board_rocket(unit_id)
    # tock=time.clock()
    # print("BOAR ROCKET",(tick-tock)*1000)
    # tick=time.clock()
    if knight_actions.boarded_rocket == False and knight_actions.moving_to_board_rocket == False:
        knight_actions.attack_moving = Utilities.attack_move(this_unit.id)
    # tock = time.clock()
    # print("ATTACK MOVE",(tick-tock)*1000)
    # tick=time.clock()
    if knight_actions.attack_moving == False and knight_actions.boarded_rocket == False and knight_actions.moving_to_board_rocket == False:
