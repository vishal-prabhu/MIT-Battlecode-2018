...
def mage_actions(unit_id):
    if not gc.can_sense_unit(unit_id):
        return
    this_unit = gc.unit(unit_id)

    config.my_units = gc.my_units()
    mage_actions.attack_moving = False
    mage_actions.boarded_rocket = False
    mage_actions.moving_to_board_rocket = False
    ...
    if gc.round() > config.rocket_launch_round and gc.planet() == bc.Planet.Earth:
        mage_actions.boarded_rocket, mage_actions.moving_to_board_rocket = Utilities.board_rocket(unit_id)
    # tock=time.clock()
    # print("BOAR ROCKET",(tick-tock)*1000)
    # tick=time.clock()
    if mage_actions.boarded_rocket == False and mage_actions.moving_to_board_rocket == False:
        mage_actions.attack_moving = Utilities.attack_move(this_unit.id)
    # tock = time.clock()
    # print("ATTACK MOVE",(tick-tock)*1000)
    # tick=time.clock()
    if mage_actions.attack_moving == False and mage_actions.boarded_rocket == False and mage_actions.moving_to_board_rocket == False:
